	...
	$.ajax({
		...
		success: function(respuesta){
			...
			if(respuesta[0]["tipo"] == "virtual"){
				...
				var detalles = JSON.parse(respuesta[0]["detalles"]); //convertir de string a array
			...
			=============================================*/
				...
				if(respuesta[0]["multimedia"] != ""){
					...
					for(var i = 0; i < imagenesMultimedia.length; i++){
						...
						//actualizo listado de fotos que se van a subir a la bd con localstorage
			...
			if(respuesta[0]["id_subcategoria"] != 0){
				...
				$.ajax({
					...
					success: function(respuesta){
						...
						//TRAER LISTADO DE SUBCATEGORIAS
						$.ajax({
			...
			$.ajax({
				...
				success: function(respuesta){
					...
					=============================================*/	
					if(respuesta["palabrasClave"] != null){

						$("#modalEditarProducto .editarPalabrasClaves").html('<div class="input-group">'+
              
                		'<span class="input-group-addon"><i class="fa fa-key"></i></span>'+ 

						'<input type="text" class="form-control input-lg tagsInput pClavesProducto" value="'+respuesta["palabrasClave"]+'" data-role="tagsinput">'+
			...
			if(respuesta == "ok"){
				...
				swal({
				  ...
				  confirmButtonText: "Cerrar"
					...
					if (result.value) {
					//remover para no tener provlemas con el nuevo producto que queramos editar
